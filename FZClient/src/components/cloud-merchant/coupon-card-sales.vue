<template>
  <div class="coupon-card" :class="{ 'lose-effect': isFailure }" style="background-color: white">
    ...
    <div class="coupon-title flex-c-m" v-if="isCouponName">{{ propsData.name }}</div>
...
export default {
  ...
  computed: {
    beginAt: function() {
      // console.log(this.propsData.beginAt);
      // return this.propsData.beginAt.substr(0, 10);
      return this.$utillib.fmtDate(new Date(this.propsData.beginAt.replace(/-/g, "/")), 'yyyy-MM-dd')
    },

    endAt: function() {
      // console.log(this.propsData.endAt)
      // return this.propsData.endAt.substr(0, 10);
      return this.$utillib.fmtDate(new Date(this.propsData.endAt.replace(/-/g, "/")), 'yyyy-MM-dd')
    }
  }
};
</script>
